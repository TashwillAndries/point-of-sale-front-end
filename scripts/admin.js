...
.then(data =>{
    ...
    let productContainer = document.querySelector('.shop-items')
        ...
        <div class="shop-item-details">
                        <span class="shop-item-price">R${product[4]}</span>
...
.then(data =>{
    ...
    function update(){
        fetch(`https://tashwill-system.herokuapp.com/edit-product/${idstored.getItem('product_id')}/`, {
            method: "PUT",
            body: JSON.stringify({
...
function convert(){
  ...
  function create(){
    ...
    fetch('https://tashwill-system.herokuapp.com/products-create/', {
        method: "POST",
        body: JSON.stringify({
            'item_name': document.querySelector('#item_name').value,
            'description':  document.querySelector('#description').value,
